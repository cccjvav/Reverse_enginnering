  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    try {
      ...
      if (error.code !== "EADDRINUSE" || attempt === 1) {
        throw error.code === "EADDRINUSE" ? new Error(conflictMessage) : error;
      }
      const identity = fileIdentity(address);
      if (await probeAlive(address)) throw new Error(conflictMessage);
...
var WorkspaceHubStore = class {
  ...
  async dispose() {
    ...
    await Promise.all(owned.map((v) => v.release()));
