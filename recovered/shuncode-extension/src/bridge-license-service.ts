...
const PAYMENT_PLAN_DISPLAY_NAMES: Readonly<Record<string, string>> = Object.freeze({
  day_1: "体验卡",
  week_1: "周卡",
  month_1: "月卡",
});
...
type BridgeAuthProvider = "github" | "gitee";

interface AccountState {
  readonly id: string;
  readonly authProvider?: BridgeAuthProvider;
  readonly githubUserId: string;
  readonly githubLogin: string;
  readonly giteeUserId: string;
  readonly giteeLogin: string;
...
export class BridgeLicenseService implements vscode.Disposable {
  ...
  async refreshSession(): Promise<BridgeAccessSnapshot> {
    try {
      await this.recoverStoredProviderSession(true, false);
      try {
        await this.refreshLicense(false);
      } catch (error) {
        const missingEntitlement = error instanceof LicenseHttpError
  ...
  private async refreshLicense(retryAuthentication: boolean): Promise<JwtClaims> {
    ...
    try {
      ...
      sessionToken = await this.recoverStoredProviderSession(false, false);
      const response = await this.request<LicenseResponse>("/v1/licenses/refresh", {
        method: "POST",
        headers: { Authorization: `Bearer ${sessionToken}` },
        body: JSON.stringify({ installationId }),
  ...
  private async ensureSessionToken(interactive: boolean): Promise<string> {
    ...
    return await this.recoverStoredProviderSession(interactive, interactive);
  }

  private storedAuthProvider(): BridgeAuthProvider {
    const account = this.context.globalState.get<AccountState>(ACCOUNT_STATE_KEY);
    if (account?.authProvider === "github" || account?.authProvider === "gitee") return account.authProvider;
    return account?.giteeUserId || account?.giteeLogin ? "gitee" : "github";
  }

  private async recoverStoredProviderSession(interactiveGitee: boolean, interactiveGitHub: boolean): Promise<string> {
    if (this.storedAuthProvider() === "gitee") {
      if (!interactiveGitee) {
        // Gitee codes cannot be renewed silently; preserve the cached license and
        // require Gitee sign-in instead of switching to a different GitHub account.
        await this.context.secrets.delete(SESSION_TOKEN_KEY);
        throw new Error("Gitee Bridge session expired. Sign in with Gitee again to renew the session.");
      }
      return await this.exchangeGiteeSession();
    }
    return await this.exchangeGitHubSession(interactiveGitHub);
  }
  ...
  private async exchangeGitHubToken(githubToken: string): Promise<string> {
    ...
    await this.persistAuthResponse(response, "github");
  ...
  private async exchangeGiteeSession(): Promise<string> {
    ...
    try {
      ...
      await this.persistAuthResponse(response, "gitee");
  ...
  private async persistAuthResponse(response: AuthResponse, authProvider: BridgeAuthProvider): Promise<void> {
    const account = this.toAccountState(response.user, authProvider);
    await Promise.all([
  ...
  private toAccountState(user: AuthResponse["user"], authProvider: BridgeAuthProvider): AccountState {
    return {
      id: user.id,
      authProvider,
  ...
  private parsePaymentPlans(response: PaymentPlansResponse): BridgePaymentPlanSnapshot[] {
    return (response.plans ?? []).flatMap((value) => {
      ...
      const name = PAYMENT_PLAN_DISPLAY_NAMES[id] ?? serverName;
      if (!id || !name) return [];
  ...
  private async storeLicenseResponse(response: LicenseResponse): Promise<void> {
    ...
    if (response.user) {
      await this.context.globalState.update(ACCOUNT_STATE_KEY, this.toAccountState(response.user, this.storedAuthProvider()));
    }
  }
  ...
  private toPaymentOrder(response: PaymentOrderResponse): BridgePaymentOrderSnapshot {
    return {
      ...
      planName: PAYMENT_PLAN_DISPLAY_NAMES[response.order.planId] ?? response.order.planName,
