...
// GPT-6 Astra requires >= 0.153.0 for catalog visibility; availability still
// comes from the authenticated server response, never a local model whitelist.
...
const CODEX_TOKEN_REQUEST_TIMEOUT_MS = 20_000;
...
async function postTokenEndpoint(body: string, contentType: string, label: string): Promise<any> {
  const target = new URL(CODEX_TOKEN_URL);
  ...
  const proxyUrl = proxySupport === "off" ? undefined : resolveExtensionHostProxy(target, configuredProxy);
  const signal = AbortSignal.timeout(CODEX_TOKEN_REQUEST_TIMEOUT_MS);
  let response: Response;
  try {
    response = await fetchWithExtensionHostFallbacks(target, {
      method: "POST",
    ...
    const detail = timedOut ? "token request timed out" : error instanceof Error ? error.message : String(error);
