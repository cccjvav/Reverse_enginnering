...
import {
  ManagedCommandCanceller,
  NATIVE_MANAGED_COMMAND_OWNER_ID,
  forceCancellationRequiresConfirmation,
  managedCommandNotAccessibleError,
  type ManagedCommandCancellationPreview,
  type ManagedCommandCancellationTarget,
} from "../../../src/managed-command-cancellation.js";
import { createManagedCommandId, normalizeManagedCommandId } from "../../../src/managed-command-id.js";
import { managedCommandStateIdsToPrune } from "../../../src/managed-command-retention.js";
import { isManagedTerminalIdleExpired, managedTerminalIdleDelay } from "../../../src/managed-terminal-lifecycle.js";
...
interface CommandState {
  ...
  endedAt?: number;
  background: boolean;
  status: "running" | "completed" | "failed" | "killed" | "cancelled";
  exitCode: number | null;
  pipelineExitCodes?: number[];
  cancelRequestedAt?: number;
  cancelForced?: boolean;
  output: Buffer;
...
function managedShellSpec(protocolToken: string): ManagedShellSpec {
  if (process.platform === "win32") {
    // Preferred shell: the product-bundled Git Bash staged under extensions/shuncode/
    // runtime/git by scripts/ensure-portable-git.mjs. One bash contract across Windows and
    // POSIX keeps agent syntax, the OSC 633 prompt protocol and the Unix toolchain
    // (git/grep/sed/awk/curl) identical everywhere, with no user-facing installation.
    const bundledBash = bundledGitBashExecutable();
    if (!bundledBash) {
      throw new Error("ShunCode managed terminal requires the product-bundled Git Bash runtime. Reinstall or rebuild ShunCode; PowerShell fallback is intentionally disabled.");
    }
    const gitRoot = path.resolve(bundledBash, "..", "..");
    return {
      kind: "bash",
      executable: bundledBash,
      args: ["--noprofile", "--norc", "-i"],
      env: {
        PROMPT_COMMAND: bashPromptCommand(protocolToken),
        PS1: "$ ",
        SHELL: bundledBash,
        PATH: withBundledGitPath(gitRoot, process.env.PATH ?? process.env.Path ?? ""),
      },
    };
  }
  ...
  if (fs.existsSync(bash)) {
    const promptCommand = bashPromptCommand(protocolToken);
    return {
      kind: "bash",
      executable: bash,
      args: ["--noprofile", "--norc", "-i"],
...
let cachedBundledGitBash: string | undefined | null;

/**
 * Product-bundled Git Bash (PortableGit, GPLv2 with its LICENSE.txt inside the payload)
 * staged under extensions/shuncode/runtime/git by scripts/ensure-portable-git.mjs.
 * Resolved relative to the compiled bundle first and then relative to the installed
 * extension layout inside the host app. Returns undefined when the payload is absent so
 * the shell silently falls back to the in-box Windows PowerShell.
 */
function bundledGitBashExecutable(): string | undefined {
  if (cachedBundledGitBash === null) return undefined;
  if (cachedBundledGitBash) return cachedBundledGitBash;
  const candidates = [
    path.join(__dirname, "..", "runtime", "git", "bin", "bash.exe"),
    path.join(vscode.env.appRoot, "extensions", "shuncode", "runtime", "git", "bin", "bash.exe"),
  ];
  for (const candidate of candidates) {
    if (fs.existsSync(candidate)) {
      cachedBundledGitBash = candidate;
      return candidate;
    }
  }
  cachedBundledGitBash = null;
  return undefined;
}

/** Prepends the PortableGit tool directories (bash, git, coreutils) to a PATH value. */
function withBundledGitPath(gitRoot: string, existingPath: string): string {
  const additions = [
    path.join(gitRoot, "usr", "bin"),
    path.join(gitRoot, "mingw64", "bin"),
    path.join(gitRoot, "bin"),
  ];
  return [...additions, existingPath].filter(Boolean).join(";");
}

/**
 * PROMPT_COMMAND payload emitting the ShunCode OSC 633 progress/exit/cwd markers after
 * every command. Shared by the POSIX shell branch and the bundled Git Bash on Windows so
 * both keep byte-identical protocol behavior.
 */
function bashPromptCommand(protocolToken: string): string {
  return [
    "__shuncode_ec=$? __shuncode_pipe=(\"${PIPESTATUS[@]}\")",
    "__shuncode_seq=$((${__shuncode_seq:-0}+1))",
    "__shuncode_cwd=$(printf '%s' \"$PWD\" | base64 | tr -d '\\r\\n')",
    "__shuncode_pipeline=$(IFS=,; printf '%s' \"${__shuncode_pipe[*]}\")",
    `printf '\\033]633;ShunCode;${protocolToken};%s;%s;%s;%s\\007' \"$__shuncode_seq\" \"$__shuncode_ec\" \"$__shuncode_cwd\" \"$__shuncode_pipeline\"`,
  ].join("; ");
}
...
class ManagedCommandPseudoterminal implements vscode.Pseudoterminal, vscode.Disposable {
  ...
  private shellKind: "bash" | "sh" | "powershell" = "powershell";
  /** Shell the persistent PTY actually launched; surfaced in the run_command result header. */
  get shellName(): "bash" | "sh" | "powershell" {
  ...
  private activeCommand: {
    ...
    finishPipelineExitCodes?: number[];
    finishTimer?: ReturnType<typeof setTimeout>;
    onOutput(text: string): void;
    onExit(code: number | null, pipelineExitCodes?: number[], meta?: { recovered?: boolean; signal?: number }): void;
  } | undefined;
  ...
  /** Interrupt only the foreground command, preserving the Bash PTY for reuse. */
  interruptActiveCommand(): boolean {
    if (this.disposed || !this.activePty || !this.activeCommand || this.activeCommand.finishing) return false;
    this.resetEchoGate();
  ...
  private async startPersistentShell(): Promise<void> {
    ...
    this.activePtyExitSubscription = ptyProcess.onExit((event) => {
      setTimeout(() => {
        ...
        activeCommand?.onExit(event.exitCode, undefined, { recovered: activeCommand.recoveryInitiated === true, signal: event.signal });
  ...
  private handleProtocolMarker(payload: string): void {
    ...
    const [sequenceText, exitCodeText, cwdBase64, pipelineExitCodesText] = payload.split(";", 4);
    ...
    activeCommand.finishPipelineExitCodes = pipelineExitCodesText === undefined
      ? undefined
      : pipelineExitCodesText === ""
        ? []
        : pipelineExitCodesText.split(",").map((value) => Number.parseInt(value, 10)).filter((value) => Number.isFinite(value));
  ...
  private finishCommand(command: NonNullable<typeof this.activeCommand>): void {
    ...
    this.restoreDisplayDimensions();
    this.continuationPromptSeen = false;
    command.onExit(command.finishExitCode ?? null, command.finishPipelineExitCodes, { recovered: command.recoveryInitiated === true });
  }
 ...
 *
 * The script flavour follows the shell the persistent terminal actually launched, not the
 * host platform: the bundled Git Bash is the preferred shell on Windows too, so a win32 host
 * normally needs a POSIX .sh. Only the PowerShell fallback keeps the .ps1 form.
 */
function writeTempScript(
  commandId: string,
  command: string,
  shellKind: "bash" | "sh" | "powershell",
): string {
  ...
  // LF endings: bash folds a trailing \r into the command itself ("$'\r': command not
  // found"), and this bridge is precisely the path taken for CRLF payloads.
...
/** Single-line invocation for a temp script. The script always runs in a child shell so an
 * `exit N` inside the payload cannot kill the persistent managed shell; the child's exit code
 * is what the prompt marker reports. */
function tempScriptCommand(scriptPath: string, shellKind: "bash" | "sh" | "powershell"): string {
  if (shellKind === "powershell") {
  ...
  if (process.platform === "win32") {
    // The persistent shell is the bundled Git Bash. Forward slashes sidestep every
    // backslash-quoting rule of the outer bash and are native to bash.exe.
...
class TerminalCommandManager implements vscode.Disposable {
  ...
  private readonly commandCanceller = new ManagedCommandCanceller();
    ...
    this.disposables.push(
      vscode.window.onDidCloseTerminal((terminal) => {
        ...
        for (const state of this.states.values()) {
          ...
          state.status = state.cancelRequestedAt === undefined ? "killed" : "cancelled";
          state.exitCode = null;
  ...
  private clearIdleTimer(slot: TerminalSlot): void {
    if (!slot.idleTimer) return;
    clearTimeout(slot.idleTimer);
    slot.idleTimer = undefined;
  }

  private scheduleIdleClose(slot: TerminalSlot): void {
    this.clearIdleTimer(slot);
    if (slot.closed || slot.busyCommandId) return;
    slot.idleTimer = setTimeout(() => {
      slot.idleTimer = undefined;
      if (slot.closed || slot.busyCommandId) return;
      if (!isManagedTerminalIdleExpired(slot.lastUsedAt)) {
        this.scheduleIdleClose(slot);
        return;
      }
      this.disposeIdleSlot(slot);
    }, managedTerminalIdleDelay(slot.lastUsedAt));
  ...
  private forceCloseSlot(slot: TerminalSlot): void {
  ...
  private pruneIdleTerminals(): void {
    ...
    for (const slot of idle) {
      const key = `${slot.ownerId}\0${this.fileSystemPathKey(this.currentSlotCwd(slot))}`;
  ...
  private async acquireTerminal(
    ...
    const candidates = [...this.slots.values()]
      .filter((slot) => slot.ownerId === ownerId && !slot.closed && !slot.busyCommandId)
      .sort((a, b) => b.lastUsedAt - a.lastUsedAt);
  ...
  private finishState(state: CommandState, exitCode: number | null, status?: CommandState["status"], meta?: { recovered?: boolean }): void {
    ...
    let finalStatus = state.cancelRequestedAt !== undefined ? "cancelled" : status ?? (exitCode === 0 ? "completed" : "failed");
    ...
    if (state.slot) {
      ...
      if (!state.slot.closed) this.scheduleIdleClose(state.slot);
    }
    ...
    state.resolveDone();
    this.pruneCommandStates();
    this.pruneIdleTerminals();
  }

  private pruneCommandStates(): void {
    const pruneIds = managedCommandStateIdsToPrune([...this.states.values()].map((state) => ({
      id: state.id,
      ownerId: state.ownerId,
      status: state.status,
      endedAt: state.endedAt,
    })));
    for (const id of pruneIds) this.states.delete(id);
  }
  ...
  private readOutput(state: CommandState, requestedOffset = 0, maxBytes = DEFAULT_OUTPUT_BYTES): Record<string, unknown> {
    ...
    return {
      ...
      pipeline_exit_codes: vscode.workspace.getConfiguration("shuncode.terminal").get<boolean>("pipelineExitCodes", true)
        ? state.pipelineExitCodes ?? null
        : null,
      cwd: state.cwd,
  ...
  async run(input: Record<string, unknown>, ownerId = NATIVE_MANAGED_COMMAND_OWNER_ID): Promise<string> {
    ...
    const cwdInfo = typeof input.cwd === "string" && input.cwd.trim()
      ...
      : undefined;
    if (input.execution !== undefined && input.execution !== "pty") {
      throw new Error("The managed terminal uses the persistent Bash PTY only; direct execution is no longer part of the public terminal contract.");
    }
    const id = createManagedCommandId();
    ...
    // Acquire the terminal first so the bridge can match the shell it actually launched:
    // on Windows that is normally the bundled Git Bash, not PowerShell.
    const { slot, reused, effectiveCwd } = await this.acquireTerminal(ownerId, id, cwdInfo);
    if (/\r|\n/.test(command) || /[^\x00-\x7F]/.test(command) || command.length > 1024) {
      const shellKind = slot.pty.shellName;
      tempScriptPath = writeTempScript(id, command, shellKind);
      execCommand = tempScriptCommand(tempScriptPath, shellKind);
    }
    ...
    try {
      ...
      await slot.pty.run(execCommand, {
        ...
        onExit: (code, pipelineExitCodes, meta) => {
          state.pipelineExitCodes = pipelineExitCodes;
          this.finishState(state, meta?.recovered ? null : code, meta?.recovered ? "failed" : undefined, meta);
        },
      }, captureColumns);
    } catch (error) {
    ...
    return [
      ...
      `execution: ${state.kind}`,
      `shell: ${slot.pty.shellName}`,
      `terminal_reused: ${reused}`,
      `command: ${JSON.stringify(command)}`,
      `status: ${snapshot.status}`,
      `exit_code: ${snapshot.exit_code ?? "null"}`,
      `pipeline_exit_codes: ${snapshot.pipeline_exit_codes === null ? "null" : JSON.stringify(snapshot.pipeline_exit_codes)}`,
  ...
  private async runDirect(
    ...
    let args: string[];
    let tempScriptPath: string | undefined;
    let directShell: "bash" | "sh" | "powershell";
    const directEnv = managedProcessEnvironment();
    if (process.platform === "win32") {
      const bundledBash = bundledGitBashExecutable();
      if (!bundledBash) throw new Error("ShunCode direct compatibility path also requires the product-bundled Git Bash runtime.");
      directShell = "bash";
      file = bundledBash;
      args = ["--noprofile", "--norc", "-c", command];
      const gitRoot = path.resolve(bundledBash, "..", "..");
      directEnv.PATH = withBundledGitPath(gitRoot, directEnv.PATH ?? directEnv.Path ?? "");
    } else {
      file = fs.existsSync("/bin/bash") ? "/bin/bash" : "/bin/sh";
      args = ["-c", command];
      directShell = file === "/bin/bash" ? "bash" : "sh";
    }
    ...
    const state: CommandState = {
      id,
      ownerId: NATIVE_MANAGED_COMMAND_OWNER_ID,
      kind: "direct",
      terminal: null,
      terminalId: "direct",
      terminalName: "direct",
  ...
  private stateForOwner(commandId: string, ownerId: string): CommandState {
    const id = normalizeManagedCommandId(commandId);
    const state = this.states.get(id);
    if (!state || state.ownerId !== ownerId) throw managedCommandNotAccessibleError();
    return state;
  }

  private cancellationTarget(state: CommandState): ManagedCommandCancellationTarget {
    const manager = this;
    return {
      id: state.id,
      ownerId: state.ownerId,
      ...
      get cancelRequestedAt() { return state.cancelRequestedAt; },
      get cancelForced() { return state.cancelForced; },
      get terminalReusable() { return Boolean(state.slot && !state.slot.closed); },
      done: state.done,
      requestInterrupt(at: number): boolean {
        if (state.status !== "running") return false;
        state.cancelRequestedAt = at;
        if (state.slot) return state.slot.pty.interruptActiveCommand();
        try { return state.child?.kill("SIGINT") === true; } catch { return false; }
      },
      forceClose(): void {
        if (state.status !== "running") return;
        state.cancelForced = true;
        if (state.slot) manager.forceCloseSlot(state.slot);
        else {
  ...
  async cancel(input: Record<string, unknown>, ownerId = NATIVE_MANAGED_COMMAND_OWNER_ID): Promise<string> {
    const state = this.stateForOwner(asString(input.command_id), ownerId);
    const target = this.cancellationTarget(state);
    const graceMs = asInteger(input.grace_ms, 1500, 100, 10_000);
    const force = asBoolean(input.force, false);
    let forceConfirmed = false;
    const preview: ManagedCommandCancellationPreview = this.commandCanceller.preview(target, ownerId);
    if (force && forceCancellationRequiresConfirmation(preview)) {
      this.commandCanceller.reserveForcePrompt(target, ownerId);
      const consequence = preview.riskConsequence ? `\n\n${preview.riskConsequence}` : "";
      const choice = await vscode.window.showWarningMessage(
        `Force-stop this ShunCode managed terminal?\n\n${preview.command}${consequence}`,
        { modal: true, detail: `High-risk command: ${preview.riskReason ?? "destructive operation"}. Only this owner-scoped terminal will be closed.` },
        "Force Stop",
      );
      if (choice !== "Force Stop") throw new Error("FORCE_CANCEL_DECLINED: local user did not approve force-closing the managed terminal.");
      forceConfirmed = true;
    }
    const result = await this.commandCanceller.cancel(target, { ownerId, graceMs, force, forceConfirmed });
    return [
      "=== CANCEL_COMMAND BEGIN ===",
      `command_id: ${state.id}`,
      ...
      `interrupt_sent: ${result.interruptSent}`,
      `force_required: ${result.forceRequired}`,
      `forced: ${result.forced}`,
      `risk: ${result.riskLevel}`,
      `terminal_reusable: ${result.terminalReusable}`,
      "=== CANCEL_COMMAND END ===",
    ].join("\n");
  }

  releaseOwner(ownerId: string): void {
    this.commandCanceller.releaseOwner(ownerId);
    for (const state of [...this.states.values()]) {
      if (state.ownerId !== ownerId) continue;
      if (state.status === "running" && state.slot) this.forceCloseSlot(state.slot);
      this.states.delete(state.id);
    }
    for (const slot of [...this.slots.values()]) if (slot.ownerId === ownerId && !slot.busyCommandId) this.disposeIdleSlot(slot);
  }

  getOutput(input: Record<string, unknown>, ownerId = NATIVE_MANAGED_COMMAND_OWNER_ID): string {
    const state = this.stateForOwner(asString(input.command_id), ownerId);
    const id = state.id;
  ...
  sendInput(input: Record<string, unknown>, ownerId = NATIVE_MANAGED_COMMAND_OWNER_ID): string {
    const state = this.stateForOwner(asString(input.command_id), ownerId);
    const id = state.id;
...
export class IdeToolBroker implements vscode.Disposable {
  ...
  async runTerminalSmokeTest(): Promise<string> {
    const firstCommand = "export SHUNCODE_PERSISTENT_SMOKE=yes; printf 'shuncode-terminal-smoke-1\\n'; printf 'bash=%s\\n' \"$BASH_VERSION\"; printf 'git=%s\\n' \"$(command -v git || true)\"";
    const secondCommand = "printf 'shuncode-terminal-smoke-2:%s\\n' \"$SHUNCODE_PERSISTENT_SMOKE\"";
  ...
  releaseCommandOwner(ownerId: string): void {
    this.terminalManager.releaseOwner(ownerId);
  }

  private async executeTool(
    name: string,
    input: Record<string, unknown>,
    commandOwnerId = NATIVE_MANAGED_COMMAND_OWNER_ID,
  ): Promise<vscode.LanguageModelToolResult> {
    switch (name) {
      case "list_directory": return toolResult(await listDirectory(input));
      case "run_command": return this.terminalResult(name, input, await this.terminalManager.run(input, commandOwnerId));
      case "get_command_output": return this.terminalResult(name, input, this.terminalManager.getOutput(input, commandOwnerId));
      case "cancel_command": return this.terminalResult(name, input, await this.terminalManager.cancel(input, commandOwnerId));
      case "send_command_input": return this.terminalResult(name, input, this.terminalManager.sendInput(input, commandOwnerId));
  ...
  private terminalResult(name: string, input: Record<string, unknown>, text: string): vscode.LanguageModelToolResult {
    ...
    switch (name) {
      ...
      case "send_command_input":
        ...
        break;
      case "cancel_command":
        result.toolResultMessage = /^status: cancelled$/m.test(text)
          ? "Command cancelled"
          : /^force_required: true$/m.test(text)
            ? "Command still running · force stop available"
  ...
  async invokeDirect(
    ...
    try {
      const result = await this.executeTool(name, args, commandOwnerId);
      if (cancellationToken?.isCancellationRequested) return { text: `IDE tool ${name} canceled.`, isError: true };
      return { text: resultText(result), isError: false };
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
