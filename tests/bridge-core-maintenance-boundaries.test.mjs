...
test('release gate refuses to equate recovered contracts or a bundle with a complete product', async()=>{
  ...
  assert.equal(report.experimentalVariant,'portable-chat-fallback');
  assert.equal(report.httpSourceIntegrated,true);
  assert.equal(report.gates.find(g=>g.id==='candidate-types').status,'PASS');
  assert.equal(report.candidateContractModules,14);
  assert.equal(report.gates.find(g=>g.id==='candidate-types').errors,0);
