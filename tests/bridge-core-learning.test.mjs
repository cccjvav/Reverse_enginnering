...
test('every line of the six explained files is mapped; coverage and lessons are fresh',async()=>{
  const {report,files}=await learningArtifacts();
  assert.equal(report.summary.fullyExplainedFiles,6);assert.equal(report.summary.explainedLines,363);
