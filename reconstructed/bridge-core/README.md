...
- **26 个可独立加载的模块 / 152 个声明 / 113,420 字节 JS**。
- 其中 **24 个自有源码标签**：23 个标签的已发布顶层声明全部纳入，`file-tool-registry` 仅纳入工具目录和输入解析；另外 2 个文件是原构建元数据、SDK 版本常量。
...
**重要限制：** `file-tool-registry.js` 没有 `dispatchFileTool` / `invokeFileTool`，本注册表不能执行读写、搜索或补丁操作。`readFiles` 已可从独立模块调用，但尚未接入调度器；搜索与写入执行器仍缺失。路径解析不等于安全隔离，原读取器还有已复现的检查/使用竞态，详见 [FILE_READER.md](FILE_READER.md)。IDE 工具仍需 VS Code API 和原宿主执行器。输入校验器实现的是 JSON Schema 的一个子集，不是完整标准验证器。
...
- `snapshot-build-metadata.js` 保留的是**原 0.7.4 发行构建**的 `version/gitSha/builtAt/release`；`getBuildInfo()` 返回该快照身份，不代表本重建包是官方发行版。本包自身版本是 `0.0.0-reconstructed.2`，且禁止 npm 发布。
...
测试状态及 Windows runner 验证以 [`docs/evidence/bridge-read-tests.json`](../../docs/evidence/bridge-read-tests.json) 为准。读取器已恢复，但不宣称能抵御恶意并发目录替换。下一阶段继续处理文件访问竞态、恢复其他实际执行链，再接回扩展入口和构建配置。
