...
> **后续更正**:实际缺的是 **11 个成员**,不是 5 个。TypeScript 对一个对象字面量
> 只报第一个多余属性,所以早先的探针少数了。多出的 6 个(`isError`、`durationMs`、
> `terminalId`、`diff`、`summary`、`detailsLabel`)同样是纯展示字段,结论不变。
> 全部 11 个已重建并发布在 `community/host-types/`。
