...
### 31.5 P1.3 第一步：把"信任宿主的补丁计划"改成"自己解析派生"

31.4修好的是Windows上键对不上的**症状**。复盘时我发现病根更深：为什么授权的正确性要取决于调用方**声明**了什么？

原设计里，`apply_patch`的操作类型（新增/修改/移动/删除）由调用方通过`patchPlan`传进来。这有两个层次的问题：

1. **脆弱**——调用方必须把路径拼写得和执行器解析结果一模一样，错一点就全盘拒绝（31.4的Windows缺陷）。
2. **不安全**——这是个真正的信任边界漏洞。调用方可以递交一份`*** Delete File: secret.txt`的补丁，同时声明`patchPlan`里这个路径是`update`。策略只看声明，于是**一个只授予"修改"权限的宿主会批准一次删除**。加再多路径规范化也堵不上，因为规范化只管键对不对得上，不管声明是不是真的。

所以这一步不是继续打补丁，而是**取消这个参数**。`derivePatchPlan()`现在用原件导出的`parsePatch`、`resolveExistingPath`、`resolveNewPath`（`apply-patch.js`第787行的导出）自己解析补丁，得出每个绝对路径对应的真实操作。操作类型来自**执行器将要执行的那次解析**，不来自任何人的声明。`invokeFileToolWithPolicy`不再接受`patchPlan`；`createPermissionCallback`保留该参数，仅供单测直接注入。

顺带解决了31.4：既然键和值都由同一套原件解析器产生，就不存在"调用方拼写和执行器解析不一致"的可能，那类问题被整类消除，而不是逐个平台去修。

**实测伪造计划**（`/tmp/hostile.mjs`，补丁体是Delete File，声明是update）：策略看到的操作是`['delete']`，调用`isError: true`，`secret.txt`内容完好。改前这会是一次被批准的删除。

解析失败一律fail-closed：计划为空Map，所有路径归为无法分类而拒绝，权威错误由执行器重新解析后报出——不自己编错误信息。

测试从18项增至20项，新增：delete被正确归类且确实删除、不可解析补丁不授权也不改文件、**调用方无法通过自带计划把delete伪装成update**、move同时授权源与目标（只批modify时目标被拒且两端都不变）、计划缺失/非Map时拒绝且**审批器调用次数为0**。

变异验证：M9（改回信任调用方计划）失败4项、M10（不记录move目标）失败1项、M11（全部记成update）失败2项、M14（分类异常时返回true）失败1项——均被捕获。

**两个存活变异，如实记录为等价变异而不是假装覆盖：** M12（解析失败时计划设为`undefined`而非空Map）与M13（删掉"计划不是Map"的显式守卫）跑完20项测试全过。我没有为了凑杀死率去写一个断言内部细节的测试，而是写了差分探针：对`undefined`/`null`/`{}`/空Map/错路径五种计划、以及两种端到端解析失败场景，记录**全部可观察输出**（回调返回值、抛出的错误码、审批器被调用次数、isError、文件内容），基线与变异体逐字节比对——**完全相同**，全部拒绝、审批器0次调用、文件完好。二者在当前代码下确实无法被任何行为测试区分，因为空Map和缺失计划走到同一个守卫、产生同一个拒绝。M13的显式守卫因此是**今天冗余、明天有用**的纵深防御，故意保留：万一以后有人重构下面的查找逻辑，它能防止"没有计划"退化成"不做检查"。

本轮验证：`npm test` **164/164**；Python 39运行/36通过/3项Windows专属跳过；`learn:check`退出0；**portable包仍75输入、产物SHA仍`635190e62a2874fa…d823d2aaa`、1677498字节，与基线逐字节相同**；`recovered/`、`reconstructed/`、`reference/`零改动。`check:release`仍退出2、7项门槛BLOCKED，`authorization-integration`维持BLOCKED——**策略至今只被维护层测试驱动，没有任何真实宿主UI构造它，portable产物里也没有它**，这一步提升的是策略本身的可信度，不是接入度。
