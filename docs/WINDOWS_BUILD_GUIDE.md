...
| 独立 Bridge 核心模块 | 37 个 JS 模块可重建/测试；读写风险已记录 | 维护与恢复基础，不是完整扩展或 VSIX |
...
2. **补共享执行链**：此前5个Custom Tools/Skills目标已恢复JS；图片读取也已独立恢复。继续补搜索、完整注册/调度器、ripgrep资产定位等链条，并建立barrel再导出。
3. **接回 TS**：37 个独立 JS 模块不自动解决原工程的 32 处缺失导入；需要正确文件布局、类型、构建配置和锁定依赖，不能用一堆 `any`/空函数伪造通过。
