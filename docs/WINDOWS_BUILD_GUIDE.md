...
| 独立 Bridge 核心模块 | 28 个 JS 模块可重建/测试；读写风险已记录 | 维护与恢复基础，不是完整扩展或 VSIX |
...
2. **补共享执行链**：5 个直接缺失的 Custom Tools/Skills 目标，以及搜索、读图、完整注册/调度器等传递依赖。
3. **接回 TS**：28 个独立 JS 模块不自动解决原工程的 32 处缺失导入；需要正确文件布局、类型、构建配置和锁定依赖，不能用一堆 `any`/空函数伪造通过。
